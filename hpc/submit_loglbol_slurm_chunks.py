...
PHOTOMETRY_SOURCE={shlex.quote(str(photometry_source))}
...
if [ "${{PHOTOMETRY_SOURCE}}" != "chimera" ]; then
  NUTS_EXTRA_ARGS+=(--photometry-source "${{PHOTOMETRY_SOURCE}}")
fi
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default="chimera",
        help="For grahspj fits, choose Chimera composite photometry or COSMOS galaxy-only photometry.",
    )
    parser.add_argument(
        "--disable-agn",
        action="store_true",
