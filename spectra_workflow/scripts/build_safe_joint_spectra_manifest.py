#!/usr/bin/env python
"""Build a safe publication-ready spectra manifest for joint photometry+spectroscopy fits.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate publication-ready Chimera composite spectra against benchmark "
...
def main() -> int:
    ...
    output_dir = (
        args.output_dir
        or WORKFLOW_ROOT / "outputs" / "safe_chimera_spectra"
    ).expanduser().resolve()
    ...
    for idx, row in enumerate(input_rows, start=1):
        ...
        try:
            ...
            safe_name = f"{safe_filename(chimera_id)}_safe_spectrum.ecsv"
    ...
    manifest_path = output_dir / "safe_chimera_spectra_manifest.csv"
    rejected_path = output_dir / "safe_chimera_spectra_rejected.csv"
